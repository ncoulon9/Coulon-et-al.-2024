...
for (i in 1:length(start_year)){
  ...
  tmp[tmp == 0] <- NA
  #tmp["BT4A"][tmp["BT4A"] == 1] <- "BT4A"
  #tmp["BT4AI"][tmp["BT4AI"] == 1] <- "BT4AI"
  #tmp["BT8"][tmp["BT8"] == 1]  <- "BT8"
...
The issue is in the Q3 and between BTS and NS-IBTS surveys

```{r}
ab <- dplyr::filter(ab, Quarter == 3)
ab <- dplyr::filter(ab, Survey  %in% c("BTS", "NS-BTS"))
```
...
map_gears <- function(data){
  ...
  for(j in 1:nrow(data)){
    ...
    gear_col<-c("#bdea5a", "#625e49", "#d4994a")
    maps::map(database = "world", ylim=c(min(data$Lat)-7,max(data$Lat)+2),xlim=c(min(data$Lon)-10,max(data$Lon)+10),fill=TRUE,col="gray")
    plot(data$Lon,data$Lat,col=gear_colab,pch=20,cex=1,xlab="Longitude",ylab="Latitude")
    maps::map(database = "world", ylim=c(min(data$Lat)-7,max(data$Lat)+2),xlim=c(min(data$Lon)-10,max(data$Lon)+10),fill=TRUE,col="gray",add=T)
    scalebar(d = 200, xy = c(min(data$Lon), min(data$Lat)), type = "bar", below = "km",lwd = 5, divs = 2, col = "black", cex = 0.85, lonlat = T)
    legend(max(data$Lon)-2.5, max(data$Lat),bg="#FFFFFF",
           legend=c("BT4A" , "BT4AI", "BT8"),
           fill =gear_col, cex = 0.85, title = "Gears")
}

map_gears(ab)
```

Looking for difference between lands

```{r}
ab <- dplyr::filter(ab, Gear != "BT8")
ab <- dplyr::filter(ab, Gear != "BT7")

levels(as.factor(ab$Country))
